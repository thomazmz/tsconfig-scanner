...
sonarMetric = 'sonarMetrics.issues'
...
# Focus on the correlation of each option with issues
issues_correlation = correlation_matrix[sonarMetric].drop(sonarMetric)

# Convert correlation results to a DataFrame for easier handling
correlation_df = issues_correlation.sort_values(ascending=False).reset_index()
correlation_df.columns = ['TypeScriptOption', 'CorrelationWithIssues']

# Plotting the correlations
plt.figure(figsize=(10, 8))
sns.barplot(data=correlation_df, x='CorrelationWithIssues', y='TypeScriptOption', palette='coolwarm')
