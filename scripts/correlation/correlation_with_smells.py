...
sonarMetric = 'sonarMetrics.smells'
...
# Focus on the correlation of each option with smells
smells_correlation = correlation_matrix[sonarMetric].drop(sonarMetric)

# Convert correlation results to a DataFrame for easier handling
correlation_df = smells_correlation.sort_values(ascending=False).reset_index()
correlation_df.columns = ['TypeScriptOption', 'CorrelationWithSmells']

# Plotting the correlations
plt.figure(figsize=(10, 8))
sns.barplot(data=correlation_df, x='CorrelationWithSmells', y='TypeScriptOption', palette='coolwarm')
